...
def three_layer_convnet(x, params):
  ...
  #conv_w1, conv_b1, conv_w2, conv_b2, fc_w, fc_b = [p.to(x.device) for p in params]
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.conv1=nn.Conv2d(in_channels=in_channel,out_channels=channel_1,kernel_size=5,stride=1,padding=2)
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net=nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(in_channels=Cin,out_channels=Cout,kernel_size=3,stride=stride,padding=1),
      nn.BatchNorm2d(Cout),
      nn.ReLU(),
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    self.cnn=nn.Sequential(
      ...
      *[ResNetStage(*arg,block=block) for arg in stage_args],
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block=nn.Sequential(
      ...
      nn.Conv2d(in_channels=Cin,out_channels=channel,kernel_size=1,stride=1,padding=0),
      nn.BatchNorm2d(channel),
      nn.LeakyReLU(negative_slope=0.1),
      nn.Conv2d(in_channels=channel,out_channels=channel,kernel_size=3,stride=stride,padding=1),
      nn.BatchNorm2d(channel),
      nn.LeakyReLU(negative_slope=0.1),
      nn.Conv2d(in_channels=channel,out_channels=Cout,kernel_size=1,stride=1,padding=0),    
    )
    if Cin==Cout and not downsample:
      self.shortcut=nn.Identity()
    else:
