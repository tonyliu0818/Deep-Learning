...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        conv1=nn.Conv2d(in_channels,stem_channels[0],3,1,1,bias=True)
        nn.init.normal_(conv1.weight,mean=0,std=0.01)
        nn.init.zeros_(conv1.bias)
        stem_cls.append(conv1)
        stem_cls.append(nn.ReLU())

        conv2=nn.Conv2d(in_channels,stem_channels[0],3,1,1,bias=True)
        ...
        for i in range(len(stem_channels)-1):
          conv1=nn.Conv2d(stem_channels[i],stem_channels[i+1],3,1,1,bias=True)
          nn.init.normal_(conv1.weight,mean=0,std=0.01)
          nn.init.zeros_(conv1.bias)
          stem_cls.append(conv1)
          stem_cls.append(nn.ReLU()) 

          conv2=nn.Conv2d(stem_channels[i],stem_channels[i+1],3,1,1,bias=True)
          nn.init.normal_(conv2.weight,mean=0,std=0.01)
          nn.init.zeros_(conv2.bias) 
          stem_box.append(conv2)
          stem_box.append(nn.ReLU())
        ...
        # Replace "pass" statement with your code
        conv_cls=nn.Conv2d(stem_channels[-1],num_classes,3, padding=1)
        nn.init.normal_(conv_cls.weight,mean=0,std=0.01)
        nn.init.zeros_(conv_cls.bias)
        self.pred_cls=conv_cls

        conv_box=nn.Conv2d(stem_channels[-1],4,3,padding=1)
        nn.init.normal_(conv_box.weight,mean=0,std=0.01)
        nn.init.zeros_(conv_box.bias)
        self.pred_box=conv_box

        conv_ctr=nn.Conv2d(stem_channels[-1],1,3,padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for i in ['p3','p4','p5']:
          ...
          centerness_logits[i]=self.pred_ctr(self.stem_box(feats_per_fpn_level[i])).reshape(b,-1,h*w).transpose(1,2)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
